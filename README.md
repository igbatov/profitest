...
2. Заменяем все нецифры на пробелы, получаем массив чисел котрые раздалены одним или несколькими пробелами.
3. Нам теперь нужно разбить этот массив на номера. 
Если массив чисел будем слишком большой, то алгоритм может работать слишком долго,
в этом случае тсходную строку мы будем отсылать на почту специально обученному человеку. 
...
7, 10 или 11 цифр. То есть каждый номер это блок длинной 7 (прямой номер), 10 (формат 905 123 45 67) или 11 цифр (8 905 123 45 67).
...
Здесь у нас получился массив из 10 чисел.
Чтобы выделить первый блок будем конкатенировать числа пока не образуется блок длины 7, 10 или 11 цифр.
Если образовался блок длины 7 - делаем ветвление дерева. Одна ветка пойдет набирать следующий блок,
вторая ветка продолжит набирать первый в расчете на то что можно добить 7 до 10 или 11.

В итоге получится набор возможных разбиений на номера.
